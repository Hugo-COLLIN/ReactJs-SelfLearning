...
function App() {
   ...
   const displayFavFruit = (fruitName) => {
      alert(`My favorite fruit is ${fruitName}!!!`)
   }
    ...
    <>
      ...
      <ul>
        {fruits.map((fruit) =>  
          ...
          // <Fruit fruitInfo={fruit} actionClick={() => displayFavFruit(fruit.nom)} key={fruit.id}/>
          <Fruit fruitInfo={fruit} onClick={() => displayFavFruit(fruit.nom)} key={fruit.id}/>
        )}
      </ul>
